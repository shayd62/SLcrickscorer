...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const sendPasswordReset = async (emailOrPhone: string) => {
    ...
    if (isEmail) {
        ...
        q = query(usersRef, where("phoneNumber", "==", emailOrPhone));
    }
    ...
    if (querySnapshot.empty) {
      ...
      // Don't throw an error to prevent account enumeration
      return;
    }
    
    const userDoc = querySnapshot.docs[0].data() as UserProfile;
    
    // Use the real email if it exists, otherwise construct the dummy one used for auth.
  ...
  const createUserProfile = async (uid: string, data: Omit<UserProfile, 'uid' | 'id'>) => {
    const userDocRefByPhone = doc(db, 'users', data.phoneNumber);
    const docSnapByPhone = await getDoc(userDocRefByPhone);

    if (docSnapByPhone.exists() && docSnapByPhone.data().isPlaceholder) {
        ...
        await updateDoc(userDocRefByPhone, profileData);
        setUserProfile({ id: data.phoneNumber, ...profileData, uid });
    } else if (docSnapByPhone.exists() && !docSnapByPhone.data().isPlaceholder) {
        throw new Error("A user with this phone number already exists.");
    } else {
        // This is a brand new user
        const profileData = { uid, ...data, isPlaceholder: false };
        await setDoc(userDocRefByPhone, profileData);
        setUserProfile({ id: data.phoneNumber, ...profileData, uid });
    }
  };
